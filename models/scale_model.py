...
class ScaleRecovery(nn.Module):
    ...
    def get_ground_mask(self, cam_points, normal_map, threshold=5):
        ...
        ones, zeros = torch.ones(b, 1, h, w), torch.zeros(b, 1, h, w)
    ...
    def forward(self, depth, K, real_cam_height):
        ...
        cv2.imwrite('mask.png', surface_normal[0].numpy() * 250)
        cv2.imwrite('mask.png', ground_mask[0][0].numpy().astype(int) * 250)
